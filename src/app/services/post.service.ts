...
export class PostService {
  ...
  //addLocalStorage
   ...
   public LoadLocal(){
    ...
    if (postsData) {
      this.Posts = JSON.parse(postsData).map((post:any) => {Object.assign(new PostModel(), post)});
    } else {
      this.Posts = [];
      this.saveLocal();
    }
  ...
  createPost(post: PostModel): Observable<PostModel> {
    ...
    this.saveLocal(); // Save the updated posts to local storage
    return of(newPost); // Simulate a successful creation with an observable
  }

  updatePost(post:PostModel):PostModel{
    const postCopy = post;
    const index = this.Posts.findIndex(post => post.id === postCopy.id);
    if (index >= 0) {
